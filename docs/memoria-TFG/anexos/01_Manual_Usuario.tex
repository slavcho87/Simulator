...
Para importa los objetos estáticos seleccionamos la opciones load from file y nos aparecerá un formulario donde podemos elegir cual es el fichero que queremos importar. Una vez importados los objetos nos aparecerá un table con todos los objetos importados.
...
Observamos que los objetos que acabamos de importar no tienen ningún tipo definido. Esto es debido ya que no sabemos de antelación cuales son los tipos de objetos estáticos ya que estos se configuran desde el simulador. Por esto el simulador nos permite dos maneras de definir cuales son los tipos de los objetos importados.

La primera manera consiste en pulsar directamente sobre el tipo del item que queremos modificar y nos aparece un select con todos los tipos disponibles.

El inconveniente que genera esta manera es que si tenemos muchos objetos es demasiado lento y laborioso definir muchos items. Por esto el simulador nos permite seleccionar una lista de items y asignarles un tipo estático.
...
Para importarlos objetos dinámicos desde un fichero tenemos que seleccionar la opción load from file y nos aparecerá un formulario que nos permite seleccionar el fichero a importar. Una vez importados los objetos nos aparecerá una tabla parecida que la tabla del paso 3.1 que nos permite asignar los tipos de objetos dinámicos a cada item.
